...
* Do not use ```$wpdb``` or ```get_posts()``` unless you have good reason.
    ...
    ```get_posts()``` actually calls ```WP_Query```, but creating a new ```WP_Query``` directly bypasses a number of filters by default. Not sure whether you need these things or not? You probably don't.
